...
public class BaselineParser implements Parser
{
	...
	public static class BaselineParserFactory implements ParserFactory
	{
		...
		public Parser getParser(List<Tree<String>> trainTrees, Indexer<FeatureIndexable> featureIndexer, Weights weights) {
			return new BaselineParser(trainTrees, featureIndexer, weights);
		}
	}
	...
	public BaselineParser(List<Tree<String>> trainTrees, Indexer<FeatureIndexable> featureIndexer, Weights weights) {
		...
		Grammar grammar = Grammar.generativeGrammarFromTrees(annotatedTrainTrees, featureIndexer, weights);
		...
		final BasicLexiconFeaturizer lexiconFeaturizer = new BasicLexiconFeaturizer();
		lexicon = new Lexicon(annotatedTrainTrees, featureIndexer, lexiconFeaturizer, weights);
		
		
		setBasicLexiconFeatureWeight(featureIndexer, weights);
	 ...
	 * @param weights
	 */
	private void setBasicLexiconFeatureWeight(Indexer<FeatureIndexable> featureIndexer, Weights weights) {
		weights.setWeight(featureIndexer.addAndGetIndex(BasicLexiconFeaturizer.DEFAULT_LEXICON_FEATURE_OBJECT), 1.0);
	}
